...
describe('flush', () => {
  test
    .nock('http://localhost:8001', api => api
      ...
      .delete(`/snis/${example.snis[0].id}`)
      .reply(204)
      .delete(`/certificates/${example.certificates[0].id}`)
      .reply(204)
      .delete(`/plugins/${example.plugins[0].id}`)
      .reply(204)
      .delete(`/upstreams/${example.upstreams[0].id}`)
      .reply(204)
      .delete(`/consumers/${example.consumers[0].id}`)
      .reply(204)
      .delete(`/routes/${example.routes[0].id}`)
      .reply(204)
      .delete(`/services/${example.services[0].id}`)
      .reply(204)
    )
    .stdout()
    .command(['flush'])
    .it('runs flush successfully', (ctx) => {
